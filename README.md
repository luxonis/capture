...
| Streams | per frame-set | max burst (approx) |
|---|---|---|
| all four (12MP RGB) | ~40 MB | ~32 frames (~1 s) |
| `--no-rgb` | ~22.5 MB | ~50–70 frames (~2 s) |
| `--rgb-resolution 1920x1080` | ~26 MB | ~45–60 frames |

For longer captures, use **streaming mode** (below) instead.

To take several bursts back-to-back (e.g. multiple static scenes), `capture_burst_loop.sh` runs N `--ram-buffer` captures in a row, waiting between bursts until device RAM has recovered (`RAM_THRESHOLD_MB`, default 2400), each into its own indexed folder, and pulls them all at the end:

```bash
CAPTURE_NAME=my-scene ./capture_burst_loop.sh <device-ip> 5 --no-rgb --num-frames 70
# -> output/..._my-scene-001_..., ..._my-scene-002_..., ...
```
